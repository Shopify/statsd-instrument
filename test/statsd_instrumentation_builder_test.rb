...
class StatsDInstrumentationBuilderTest < Minitest::Test
  ...
  def test_statsd_count_if
    StatsD.instrument(ActiveMerchant::Gateway) do |i|
      i.count_if(:ssl_post, 'ActiveMerchant.Gateway.if')
    end
  ...
  def test_statsd_count_success
    StatsD.instrument(ActiveMerchant::Gateway) do |i|
      i.count_success(:ssl_post, 'ActiveMerchant.Gateway', sample_rate: 0.5)
    end
  ...
  def test_statsd_count
    StatsD.instrument(ActiveMerchant::Gateway) do |i|
      i.count(:ssl_post, 'ActiveMerchant.Gateway.ssl_post')
    end
  ...
  def test_statsd_measure
    StatsD.instrument(ActiveMerchant::UniqueGateway) do |i|
      i.measure(:ssl_post, 'ActiveMerchant.Gateway.ssl_post', sample_rate: 0.3)
    end
  ...
  def test_statsd_distribution
    StatsD.instrument(ActiveMerchant::UniqueGateway) do |i|
      i.distribution(:ssl_post, 'ActiveMerchant.Gateway.ssl_post', sample_rate: 0.3)
    end
  ...
  def test_instrumenting_class_method
    StatsD.instrument(ActiveMerchant::Gateway) do |i|
      i.count(:sync, 'ActiveMerchant.Gateway.sync', singleton: true)
    end
