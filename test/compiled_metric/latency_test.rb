...
class CompiledMetricLatencyTest < Minitest::Test
  def setup
    super
  ...
  def test_distribution_without_define
    metric = Class.new(StatsD::Instrument::CompiledMetric::Latency)

    error = assert_raises(ArgumentError) do
      metric.measure(5)
    end
    assert_equal("Every CompiledMetric subclass needs to call `define` before first invocation of measure.", error.message)
  end

  def test_define_distribution_without_tags
    metric = Class.new(StatsD::Instrument::CompiledMetric::Latency) do
      define(name: "foo.bar")
    end
  ...
  def test_define_distribution_with_dynamic_tags
    ...
    metric.measure(1, shop_id: 123, user_id: 456)
  ...
  def test_define_distribution_with_string_tags
    ...
    metric.measure(2, country: "US", region: "West")
  ...
  def test_multiple_distributions_same_tags
    ...
    metric.measure(1, shop_id: 123)
    metric.measure(2, shop_id: 123)
    metric.measure(3, shop_id: 123)
  ...
  def test_multiple_distributions_different_tags
    ...
    metric.measure(1, shop_id: 123)
    metric.measure(1, shop_id: 456)
    metric.measure(1, shop_id: 789)
  ...
  def test_latency_as_value_when_block_provided
    ...
    returned_value = metric.measure(shop_id: 123, user_id: 456) do
      4
    end
  ...
  def test_latency_as_value_when_block_provided_with_only_static_tags
    ...
    returned_value = metric.measure do
      4
    end
  ...
  def test_ignores_explicit_value_when_block_provided
    ...
    returned_value = metric.measure(42, shop_id: 123, user_id: 456) {}
...
class CompiledMetricDistributionWithAggregationTest < Minitest::Test
  ...
  def test_aggregates_values_with_blocks
    ...
    first_returned_value = metric.measure(shop_id: 123, user_id: 456) do
      1
    end

    second_returned_value = metric.measure(shop_id: 123, user_id: 456) do
      2
    end
  ...
  def test_aggregates_with_values_and_blocks_mixed
    ...
    metric.measure(42, shop_id: 123, user_id: 456)
    second_returned_value = metric.measure(shop_id: 123, user_id: 456) do
      2
    end
